...
def read_data(infile, freqs, delay_ind, time_inds, red_group_list):
    ...
    hd = VisClean(infile)
    frequencies = np.logical_and(hd.freqs >= min(freqs), hd.freqs <= max(freqs))
    hd.read(frequencies=frequencies)
    hd.fft_data(ax="freq", window="bh", ifft=True, edgecut_lo=0, edgecut_hi=0,
                overwrite=True)

    data = np.array([hd.dfft[bl][min(time_inds):max(time_inds), delay_ind]
...
if __name__ == "__main__":
    ...
    noise_use = get_noise_func(args.noise_file, args.freqs, args.delay_ind,
                               args.time_inds, red_group_list)
